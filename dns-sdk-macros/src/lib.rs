...
macro_rules! extract_params {
    ...
    (@extract_required $builder:expr, $field:ident, &str) => {
        $builder.$field.clone().as_deref().ok_or_else(|| format!("{} is required", stringify!($field)))?.to_string()
    };
    (@extract_required $builder:expr, $field:ident, u32) => {
    ...
    ($builder:expr, $struct_name:ident, {
        ...
        // Generate a temporary struct for serialization with renamed fields
        #[derive(Debug, serde::Serialize)]
        struct $struct_name {
            $( #[serde(rename = $req_key)] $req_field: extract_params!(@type $req_ty), )*
            $( #[serde(rename = $opt_key)] $opt_field: extract_params!(@type $opt_ty), )*
        }

        // Construct the struct using extracted field values
        $struct_name {
